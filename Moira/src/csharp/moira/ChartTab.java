...
class ChartTab {
    ...
    private InjCalendarSpinner birth, now;

    //private LocationSpinner location;
    private InjLocationSpinner location;
    ...
    private String pick_degree, pole_degree;
    ...
    private CacheEntry inject_record = new CacheEntry();
    

    //public Composite createTabFolderPage(CTabFolder tab_folder)
    public void createTabFolderPage()
    {
    	/*
        ...
        birth = new InjCalendarSpinner();          
        birth.init(false); //傳這個false實際上沒有用處

        
        int[] birth_date = new int[5];
        birth_date[0] = Moira.mri.birth_year;
        birth_date[1] = Moira.mri.birth_month;
        birth_date[2] = Moira.mri.birth_day;
        birth_date[3] = Moira.mri.birth_hour;
        birth_date[4] = Moira.mri.birth_minute;
        birth.setCalendar(birth_date); //checkpoint:設定birth這個物件的時間(原本是SWT的UI物件)
        ...
        location = new InjLocationSpinner();
        ...
        now = new InjCalendarSpinner();
        now.init(false);
        int[] now_date = new int[5];
        now_date[0] = Moira.mri.now_year;
        now_date[1] = Moira.mri.now_month;
        now_date[2] = Moira.mri.now_day;
        now_date[3] = Moira.mri.now_hour;
        now_date[4] = Moira.mri.now_minute;
        now.setCalendar(now_date); //checkpoint:設定時間
                    ...
                    if (tip.hasDataFromPoint(event.x, event.y)) {
                        ...
                        if (tip.isDoubleValid(degree)) {
                            ...
                            int[] date = data.getDateAtSunPos(degree,
                                    ...
                                    (event.stateMask & SWT.SHIFT) != 0);
                            if (date != null) {
                                setBirthDate(date);  //checkpoint: UI entry point
                                update(true, false);
                            }
                            return true;
                        }
            ...
            boolean set_now, boolean local)
    {
        ...
        if (set_now)
            ...
            birth.setCalendar(date);  //checkpoint
        update(false, false);
    }
    ...
    public void setMountainPos(String pos)
    {
    	...
    	pick_degree = data.setMountainPos(pos);
    }
    ...
    private String savePoleDegree(int[] date)
    {
        ...
        double degree = City.parseLongLatitude(pole_degree, 'E', 'W');
        if (degree == City.INVALID)
            degree = 0.0;
        //pole_degree.setText(City.formatLongLatitude(degree, true, true, false));
        pole_degree = City.formatLongLatitude(degree, true, true, false);
    ...
    public void compute()
    {
        ...
        CacheEntry entry = getCacheEntry();  //cyliu: 依astroMode抓取cache內容，無內容則新增一筆
        ChartMode.setSingleWheelMode(entry.single);
        if (entry.tip == null)
        ...
        if (ChartMode.isMultipleMode(false)) {
            ...
            if (ChartMode.isAstroMode(ChartMode.RELATIONSHIP_MODE)) {
                Calculate.computeMidPoint(birth_entry.entry, alt_entry.entry,
                        entry.entry);
                location.setAllowCityMatch(false);						//cyliu
        ...
        pick_degree = data.getMountainPos(false);
        //setBirthGroupName();
        //refresh(false);
        //for (int i = 0; i < NUM_TAB; i++)
            //tabs[i].update();
    ...
    public class CacheEntry {
        ...
        public void save(boolean partial)  //cyliu
        {
            if (entry == null)
                entry = new DataEntry();
            // name and sex is eiter default or already been saved
            int[] date = new int[5];
            birth.getCalendar(date);  //checkpoint
    ...
    public void injectRecord(String countryName, String cityName, String zoneName)  //added by Cyliu
    {
    	DataEntry d_entry = new DataEntry();
    	d_entry.setCountry(countryName);
    	d_entry.setCity(cityName);
    	d_entry.setZone(zoneName);
    	location.setCountryName(countryName);
    	location.setCityName(cityName);
    	location.setZoneName(zoneName);
    	inject_record.save(d_entry);
    	cache_record[ChartMode.NATAL_MODE] = inject_record;
    }
}