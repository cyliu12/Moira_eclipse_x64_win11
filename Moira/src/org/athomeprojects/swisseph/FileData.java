...
class FileData {
    ...
    int sweph_denum; /*
                      * DE number of JPL ephemeris, which this file is derived
                      * from.
                      */
    ...
    // Achtung: read_const muss aufgerufen werden von "swed.fidat[ifno]", wobei
    // "ifno" ausserdem der erste Parameter in read_const sein muss:
    //  struct FileData *fdp = &swed.fidat[ifno];
    //
    // Byteorder etc. sind in Java eindeutig, so dass die diesbezueglichen
    // Informationen und Tests entfallen koennen.
    //
    /*
    ...
    int read_const(int ifno, StringBuffer serr, SwissData swed)
    {
        ...
        int i, j, ipli, kpl;
        int retc;
        // Jimmy: add byte ordering detection (code fragment from 1.72.00 java
        // port)
        int fendian, freord;
        int lastnam = 19;
        ...
        try {
            ...
            try {
                ...
                fptr.close();
                throw new SwissephException(SwissephException.DATA_FILE_ERROR,
                        SweConst.ERR, serr);
            }
            // Use SwissLib.atoi(...) ?
            fversion = ver;
            ...
            do {
                ...
                s += (char) b;
                    ...
                    && s.length() < SwissData.AS_MAXCH);
            s2 = fnam.substring(fnam.lastIndexOf(swed.DIR_GLUE) + 1)
                    .toLowerCase();
            ...
            if (!s.equals(s2)) {
                ...
                if (!s.equals(s2)) {
                    s2 = fnam.substring(fnam.lastIndexOf(swed.DIR_GLUE) + 1)
                            .toLowerCase();
                    System.out.println("Ephemeris file name '" + s2
                            + "' is wrong; rename to '" + s + "'");
                    fptr.close();
                    throw new SwissephException(
                            SwissephException.DATA_FILE_ERROR, SweConst.ERR,
                            serr);
                }
            }
            /*******************************************************************
            ...
            if (ifno == SwephData.SEI_FILE_ANY_AST) {
                ...
                /*
                 * MPC number and name; will be analyzed below: search "asteroid
                 * name"
                 */
                i = 5;
                while (s.charAt(i) != ' ') {
            ...
            // Jimmy: add byte ordering detection (code fragment from 1.72.00
            // java port)
            /*******************************************************************
             ...
             ******************************************************************/
            //lng = (long) fptr.readInt();
            //if (lng < 0) {
            //    lng += 256 * 256 * 256 * 256;
            //}
            lng = (long) read4(fptr, SwephData.SEI_CURR_FPOS, false, freord,
                    fendian);
            if (lng < 0) {
                lng &= 0xffffffff;
            }
            ...
            npl = nplan;
            /* which ones? */
            // Hopefully, I understood this mechanism correctly...
            //Old:
            //do_fread((void *) fdp->ipl, 2, (int) nplan, 2, fp,...
            //New:
            //do_fread((void *) fdp->ipl, nbytes_ipl, (int) nplan, sizeof(int),
            // fp,...
            //          target size count in what size
            //                                                  should it be returned?
            if (nbytes_ipl == 2) {
                for (i = 0; i < nplan; i++) {
                    //ipl[i] = (int) (fptr.readUnsignedByte()) * 256
                    //        + (int) (fptr.readUnsignedByte());
                    ipl[i] = (int) (read2(fptr, SwephData.SEI_CURR_FPOS,
                            freord, fendian));
                }
            } else if (nbytes_ipl == 4) {
                for (i = 0; i < nplan; i++) {
                    //ipl[i] = fptr.readInt();
                    ipl[i] = read4(fptr, SwephData.SEI_CURR_FPOS, false,
                            freord, fendian);
                }
            } else { // Can't be???
            ...
            if (ifno == SwephData.SEI_FILE_ANY_AST) {
                ...
                sastno = sastnam.substring(0, Math.min(sastnam.length(), j))
                        .trim();
                i = Integer.parseInt(sastno);
                if (i == ipl[0] - SweConst.SE_AST_OFFSET) {
                    /* element record is from bowell database */
                    astnam = sastnam.substring(Math
                            .min(sastnam.length(), j + 1), Math.min(sastnam
                            .length(), j + 1 + lastnam));
            ...
            fpos = fptr.getFilePointer();
            /* read CRC from file */
            //ulng = (long) (fptr.readInt() & 0xffffffff);
            ulng = (long) (read4(fptr, SwephData.SEI_CURR_FPOS, false, freord,
                    fendian));
             ...
             */
            //swed.gcdat.clight = fptr.readDouble();
            //swed.gcdat.aunit = fptr.readDouble();
            //swed.gcdat.helgravconst = fptr.readDouble();
            //swed.gcdat.ratme = fptr.readDouble();
            //swed.gcdat.sunradius = fptr.readDouble();
            swed.gcdat.clight = read8(fptr, SwephData.SEI_CURR_FPOS, freord,
                    fendian);
            swed.gcdat.aunit = read8(fptr, SwephData.SEI_CURR_FPOS, freord,
                    fendian);
            swed.gcdat.helgravconst = read8(fptr, SwephData.SEI_CURR_FPOS,
                    freord, fendian);
            swed.gcdat.ratme = read8(fptr, SwephData.SEI_CURR_FPOS, freord,
                    fendian);
            swed.gcdat.sunradius = read8(fptr, SwephData.SEI_CURR_FPOS, freord,
                    fendian);
            /*******************************************************************
            ...
            for (kpl = 0; kpl < npl; kpl++) {
                ...
                //pdp.lndx0 = (long) fptr.readInt();
                pdp.lndx0 = (long) (read4(fptr, SwephData.SEI_CURR_FPOS, false,
                        freord, fendian)) & 0xffffffffL;
                // Ein negativer integer bleibt auch als long negativ. Abhilfe:
                if (pdp.lndx0 < 0) {
                    pdp.lndx0 += 2147483648L;
                }
                /* flags: helio/geocentric, rotation, reference ellipse */
                pdp.iflg = fptr.readUnsignedByte();
                pdp.ncoe = fptr.readUnsignedByte();
                //lng = fptr.readInt();
                lng = read4(fptr, SwephData.SEI_CURR_FPOS, false, freord,
                        fendian);
                pdp.rmax = (double) lng / 1000.0;
                /* start and end epoch of planetary ephemeris, */
                /* segment length, and orbital elements */
                //pdp.tfstart = fptr.readDouble();
                //pdp.tfend = fptr.readDouble();
                //pdp.dseg = fptr.readDouble();
                //pdp.nndx = (int) ((pdp.tfend - pdp.tfstart + 0.1) /
                // pdp.dseg);
                //pdp.telem = fptr.readDouble();
                //pdp.prot = fptr.readDouble();
                //pdp.dprot = fptr.readDouble();
                //pdp.qrot = fptr.readDouble();
                //pdp.dqrot = fptr.readDouble();
                //pdp.peri = fptr.readDouble();
                //pdp.dperi = fptr.readDouble();
                pdp.tfstart = read8(fptr, SwephData.SEI_CURR_FPOS, freord,
                        fendian);
                pdp.tfend = read8(fptr, SwephData.SEI_CURR_FPOS, freord,
                        fendian);
                pdp.dseg = read8(fptr, SwephData.SEI_CURR_FPOS, freord, fendian);
                pdp.nndx = (int) ((pdp.tfend - pdp.tfstart + 0.1) / pdp.dseg);
                pdp.telem = read8(fptr, SwephData.SEI_CURR_FPOS, freord,
                        fendian);
                pdp.prot = read8(fptr, SwephData.SEI_CURR_FPOS, freord, fendian);
                pdp.dprot = read8(fptr, SwephData.SEI_CURR_FPOS, freord,
                        fendian);
                pdp.qrot = read8(fptr, SwephData.SEI_CURR_FPOS, freord, fendian);
                pdp.dqrot = read8(fptr, SwephData.SEI_CURR_FPOS, freord,
                        fendian);
                pdp.peri = read8(fptr, SwephData.SEI_CURR_FPOS, freord, fendian);
                pdp.dperi = read8(fptr, SwephData.SEI_CURR_FPOS, freord,
                        fendian);
                ...
                if ((pdp.iflg & SwephData.SEI_FLG_ELLIPSE) != 0) {
                    ...
                    for (i = 0; i < 2 * pdp.ncoe; i++) {
                        //pdp.refep[i] = fptr.readDouble();
                        pdp.refep[i] = read8(fptr, SwephData.SEI_CURR_FPOS,
                                freord, fendian);
                    }
                }/**/
            }
     ...
     * Ethernet CRC, i.e., BigEndian/LittleEndian byte/bit order. The CRCs this
     * code generates agree with the vendor-supplied Verilog models of several
     * of the popular FDDI "MAC" chips.
     */
    /* unsigned long [...] */
    ...
    long swi_crc32(/* unsigned??? */byte[] buf, int len)
    {
        ...
        for (pn = 0; len > 0; ++pn, --len) {
            crc = ((crc << 8) & 0xffffffffL)
                    ^ crc32_table[(int) ((crc >> 24) ^ ((long) buf[pn] & 0xff))];
        }
    ...
    // Jimmy: add byte ordering functions (code fragment from 1.72.00
    // java port)
            ...
            throws IOException, EOFException
    {
        ...
        if (freord != 0) {
            ...
            boolean do_shift = ((fendian == SwephData.SEI_FILE_BIGENDIAN && freord != 0) || (fendian == SwephData.SEI_FILE_LITENDIAN && freord == 0));
            return (do_shift ? val >> 8 : val);
        }
        return val;
    }

    private int read4(FilePtr fp, long fpos, boolean unsigned, int freord,
            int fendian) throws IOException, EOFException
    {
        if (fpos >= 0) {
            fp.seek(fpos);
        }
    ...
    // Jimmy: function from 1.72.00 java port
    /*
        ...
        try {
            ...
            for (icoord = 0; icoord < 3; icoord++) {
                ...
                if ((c[0] & 128) != 0) {
                    ...
                    nsize[4] = (int) c[3] / 16;
                    nsize[5] = (int) c[3] % 16;
                    nco = nsize[0] + nsize[1] + nsize[2] + nsize[3] + nsize[4]
                            + nsize[5];
                } else {
                    nsizes = 4;
                    nsize[0] = (int) c[0] / 16;
                ...
                if (nco > pdp.ncoe) {
                    if (serr != null) {
                        serr.append("error in ephemeris file " + fdp.fnam
                                + ": " + nco + " coefficients instead of "
                                + pdp.ncoe + ". ");
                    }
                    throw new SwissephException(
                            SwissephException.DATA_FILE_ERROR, SweConst.ERR,
                    ...
                    if (i < 4) {
                        ...
                        for (kCnt = 0; kCnt < k; kCnt++) {
                            switch (j) {
                                case 1:
                                    ...
                                    break;
                                case 2:
                                    longs[kCnt] = (int) read2(fptr,
                                            SwephData.SEI_CURR_FPOS, freord,
                                            fendian);
                                    if (longs[kCnt] < 0) {
                                        longs[kCnt] &= 0xffff;
                                    }
                                ...
                                case 3:
                                    ...
                                    break;
                                case 4:
                                    longs[kCnt] = read4(fptr,
                                            SwephData.SEI_CURR_FPOS, false,
                                            freord, fendian);
                                    if (longs[kCnt] < 0) {
                        ...
                        for (m = 0; m < k; m++, idbl++) {
                            if ((longs[m] & 1) != 0) { /* will be negative */
                                pdp.segp[idbl] = -((((longs[m] + 1) / 2) & 0x7fffffffL)
                                        / 1e+9 * pdp.rmax / 2);
                            } else {
                                pdp.segp[idbl] = ((longs[m] / 2) & 0x7fffffffL)
                                        / 1e+9 * pdp.rmax / 2;
                            }
                        }
                        ...
                        for (m = 0, j = 0; m < k && j < nsize[i]; m++) {
                            for (n = 0, o = 16; n < 2 && j < nsize[i]; n++, j++, idbl++, longs[m] %= o, o /= 16) {
                                if ((longs[m] & o) != 0) {
                                    pdp.segp[idbl] = -(((longs[m] + o) / o / 2)
                                            * pdp.rmax / 2 / 1e+9);
                                } else {
                                    pdp.segp[idbl] = (longs[m] / o / 2)
                                            * pdp.rmax / 2 / 1e+9;
                                }
                            }
                        }
