...
public class ChartData {
    ...
    private void initSign()
    {
        ...
        num_asteroids = Calculate.getAsteroidData(null, null, null,
                asteroid_list);
        signs = Resource.getStringArray("signs");	//�摰�, �鞎�, 蝳噸, 摰正, �蝘�, ����, 憭怠氖, 憟游��, �憟�, �摰�, ����, 鞎∪��
        if (num_asteroids > 0) {
            ...
            if (birth_data) {
                ...
                if (!ChartMode.isChartMode(ChartMode.ASTRO_MODE)) {
                    ...
                    String[] array = (ChartMode
                            ...
                            : stellar_names;	//sidereal_stellar_names => �����漲�嚗�甇脣榆靽格迤 //stellar_names��隞�憭拇�飛��挪��迂嚗��雿蔭撠望�革撌桐耨甇� 
                    double offset = 0.0;
                    if (house_mode) {
                        if (Resource
                                .getPrefInt(ChartMode
    ...
    private String getGauquelin(int planet_no) //cyliu:Gauquelin = �雿��
    {
        if (!show_gauquelin)
            return "";
            ...
            boolean day_pole)
    {
        ...
        key = Resource.getString("star_sky_key") + main_pole.substring(0, 1);	//天干        天干甲=寅:祿勳, 未:天貴, 丑:玉貴, 巳:天廚, 巳:文昌, 卯:陽刃, 酉:飛刃, 戌:國印, 午:紅豔, 酉:流霞, 亥:學堂, 寅:福貴, 酉:官貴
        array = Resource.getStringArray(key);
        if (array != null) {
            for (int i = 0; i < array.length; i++)
                addStarSign(head, array[i]);
        ...
        key = Resource.getString("star_earth_key") + main_pole.substring(1, 2);	//star_earth_key=地支        地支子=子:歲駕, 卯:紅鸞, 	
        array = Resource.getStringArray(key);
        if (array != null) {
            for (int i = 0; i < array.length; i++)
                addStarSign(head, array[i]);	//加入同位神煞  eg. 歲駕=劍鋒, 伏屍, 太歲
        }
        ...
        if (!day_pole) {
			...
			if (poles == birth_poles) { // add by cyliu
				Moira.mri.shen_sha_table_b = (Hashtable) table.clone();
			} else {
				Moira.mri.shen_sha_table_n = (Hashtable) table.clone();
			}          
    ...
    private void addStarSign(LinkedList head, String entry)	//entry eg. 子:歲駕
    {
    ...
    public void copyToMRI(){
		Moira.mri.result.age_label = age_label;
		Moira.mri.result.alt_birth_year_info = alt_birth_year_info;
		Moira.mri.result.alt_current_year_info = alt_current_year_info;
		Moira.mri.result.alt_pick_year_info = alt_pick_year_info;
		Moira.mri.result.alt_pole_data = alt_pole_data;
		...
		Moira.mri.result.aspects_index = aspects_index;
		Moira.mri.result.aspects_orb = aspects_orb;
		Moira.mri.result.aspects_sign = aspects_sign;
		Moira.mri.result.aspects_style = aspects_style;
		Moira.mri.result.asteroid_list = asteroid_list;
		Moira.mri.result.astro_cusp_data = astro_cusp_data;
		Moira.mri.result.astro_elemental = astro_elemental;
		Moira.mri.result.astro_elemental_state = astro_elemental_state;
		Moira.mri.result.astro_sign_data = astro_sign_data;
		Moira.mri.result.azimuth_speed = azimuth_speed;
		Moira.mri.result.bad_styles = bad_styles;
		Moira.mri.result.birth = birth;
		Moira.mri.result.birth_adj_date = birth_adj_date;
		Moira.mri.result.birth_base_time = birth_base_time;
		Moira.mri.result.birth_cusp = birth_cusp;
		Moira.mri.result.birth_cusp_override = birth_cusp_override;
		Moira.mri.result.birth_dst_date = birth_dst_date;
		Moira.mri.result.birth_loc = birth_loc;
		Moira.mri.result.birth_poles = birth_poles;	//{壽元干支、子平小運長生干支、年柱、月柱、日柱、時柱}
		Moira.mri.result.birth_ref_ut = birth_ref_ut;
		Moira.mri.result.birth_rise_time = birth_rise_time;
		Moira.mri.result.birth_set_time = birth_set_time;
		Moira.mri.result.birth_sign_alt = birth_sign_alt;
		Moira.mri.result.birth_sign_azimuth = birth_sign_azimuth;
		Moira.mri.result.birth_sign_diameter = birth_sign_diameter;
		Moira.mri.result.birth_sign_display_sort_orders = birth_sign_display_sort_orders;
		Moira.mri.result.birth_sign_pos = birth_sign_pos;
		Moira.mri.result.birth_sign_state = birth_sign_state;
		Moira.mri.result.birth_sign_status = birth_sign_status;
		Moira.mri.result.birth_sign_status_fill = birth_sign_status_fill;
		Moira.mri.result.birth_sign_tip = birth_sign_tip;
		Moira.mri.result.birth_speed_color = birth_speed_color;
		Moira.mri.result.birth_table = birth_table;
		Moira.mri.result.birth_year_info = birth_year_info;
		Moira.mri.result.birth_zone = birth_zone;
		Moira.mri.result.cal = cal;
		Moira.mri.result.calendar = calendar;
		Moira.mri.result.center_tip = center_tip;
		// Moira.mri.result.CERES=CERES;
		// Moira.mri.result.CH_MONTH=CH_MONTH;
		// Moira.mri.result.CH_YEAR=CH_YEAR;
		...
		Moira.mri.result.cur_loc = cur_loc;
		Moira.mri.result.current_age = current_age;
		Moira.mri.result.current_year_info = current_year_info;
		Moira.mri.result.cusps_name = cusps_name;
		Moira.mri.result.day_birth = day_birth;
		Moira.mri.result.day_birth_override = day_birth_override;
		Moira.mri.result.day_char = day_char;
		Moira.mri.result.day_or_night = day_or_night;
		// Moira.mri.result.DAY_POLE=DAY_POLE;
		Moira.mri.result.day_pole_base = day_pole_base;
		Moira.mri.result.day_pole_long_life_seq = day_pole_long_life_seq;
		Moira.mri.result.day_set = day_set;
		Moira.mri.result.day_table = day_table;
		// Moira.mri.result.DEGREE=DEGREE;
		Moira.mri.result.draw_size = draw_size;
		Moira.mri.result.dual_ring = dual_ring;
		Moira.mri.result.earth_god_seq = earth_god_seq;
		Moira.mri.result.earth_pole_names = earth_pole_names;
		Moira.mri.result.eight_char_override = eight_char_override;
		Moira.mri.result.equator_rise_set = equator_rise_set;
		Moira.mri.result.equatorial_orbit = equatorial_orbit;
		Moira.mri.result.eval_tab = eval_tab;
		Moira.mri.result.explain = explain;
		Moira.mri.result.explain_fixstar = explain_fixstar;
		Moira.mri.result.explain_length = explain_length;
		Moira.mri.result.explain_prefix = explain_prefix;
		Moira.mri.result.explain_star = explain_star;
		Moira.mri.result.explain_suffix = explain_suffix;
		Moira.mri.result.first_cusp_pos = first_cusp_pos;
		Moira.mri.result.fixstar_names = fixstar_names;
		// Moira.mri.result.FIXSTAR_OFFSET=FIXSTAR_OFFSET;
		Moira.mri.result.fixstar_sign_alt = fixstar_sign_alt;
		Moira.mri.result.fixstar_sign_azimuth = fixstar_sign_azimuth;
		Moira.mri.result.fixstar_sign_pos = fixstar_sign_pos;
		Moira.mri.result.fixstar_sign_state = fixstar_sign_state;
		Moira.mri.result.fixstar_signs = fixstar_signs;
		Moira.mri.result.fixstar_table = fixstar_table;
		Moira.mri.result.fly_seq_half_shift = fly_seq_half_shift;
		Moira.mri.result.fly_seq_yang1 = fly_seq_yang1;
		Moira.mri.result.fly_seq_yang2 = fly_seq_yang2;
		Moira.mri.result.fly_seq_ying1 = fly_seq_ying1;
		Moira.mri.result.fly_seq_ying2 = fly_seq_ying2;
		...
		Moira.mri.result.half_stellar_signs = half_stellar_signs;
		Moira.mri.result.has_day_birth_override = has_day_birth_override;
		Moira.mri.result.has_pos_adj = has_pos_adj;
		Moira.mri.result.horiz_sign_data = horiz_sign_data;
		// Moira.mri.result.HORIZ_ZOOM=HORIZ_ZOOM;
		// Moira.mri.result.HORIZ_ZOOM_GAP=HORIZ_ZOOM_GAP;
		Moira.mri.result.hour_char = hour_char;
		Moira.mri.result.hour_earth_pole_shift = hour_earth_pole_shift;
		// Moira.mri.result.HOUR_POLE=HOUR_POLE;
		Moira.mri.result.hour_sky_pole_shifts = hour_sky_pole_shifts;
		Moira.mri.result.house_name = house_name;
		Moira.mri.result.house_override = house_override;
		// Moira.mri.result.INV_TRUE_NODE=INV_TRUE_NODE;
		Moira.mri.result.invalid_sign = invalid_sign;
		// Moira.mri.result.JUNO=JUNO;
		// Moira.mri.result.JUPITER=JUPITER;
		Moira.mri.result.last_year_birth = last_year_birth;
		Moira.mri.result.life_sign_pos = life_sign_pos;
		Moira.mri.result.limit_seq = limit_seq;
		Moira.mri.result.long_life_signs = long_life_signs;
		Moira.mri.result.lunar_calendar = lunar_calendar;
		Moira.mri.result.lunar_date = lunar_date;
		Moira.mri.result.lunar_eclipse = lunar_eclipse;
		Moira.mri.result.lunar_leap_month = lunar_leap_month;
		Moira.mri.result.lunar_return_pos = lunar_return_pos;
		Moira.mri.result.lunar_transits = lunar_transits;
		Moira.mri.result.magnetic_shift = magnetic_shift;
		Moira.mri.result.magnetic_shift_message = magnetic_shift_message;
		// Moira.mri.result.MARS=MARS;
		Moira.mri.result.master_table = master_table;
		// Moira.mri.result.MAX_ADVANCE=MAX_ADVANCE;
		// Moira.mri.result.MAX_DATA_COUNT=MAX_DATA_COUNT;
		// Moira.mri.result.MAX_TIP_COUNT=MAX_TIP_COUNT;
		// Moira.mri.result.MC=MC;
		// Moira.mri.result.MEAN_APOG=MEAN_APOG;
		// Moira.mri.result.MERCURY=MERCURY;
		// Moira.mri.result.MILLISECOND_PER_YEAR=MILLISECOND_PER_YEAR;
		// Moira.mri.result.MIN_ADVANCE=MIN_ADVANCE;
		Moira.mri.result.month_char = month_char;
		Moira.mri.result.month_earth_pole_shift = month_earth_pole_shift;
		// Moira.mri.result.MONTH_POLE=MONTH_POLE;
		Moira.mri.result.month_sky_pole_shifts = month_sky_pole_shifts;
		// Moira.mri.result.MOON=MOON;
		Moira.mri.result.mountain_mode = mountain_mode;
		Moira.mri.result.mountain_offset = mountain_offset;
		Moira.mri.result.mountain_pos = mountain_pos;
		Moira.mri.result.mountain_signs = mountain_signs;
		// Moira.mri.result.NEPTUNE=NEPTUNE;
		Moira.mri.result.new_moons = new_moons;
		Moira.mri.result.no_color = no_color;
		Moira.mri.result.now = now;
		Moira.mri.result.now_adj_date = now_adj_date;
		Moira.mri.result.now_base_time = now_base_time;
		Moira.mri.result.now_cusp = now_cusp;
		Moira.mri.result.now_date = now_date;
		Moira.mri.result.now_degree = now_degree;
		Moira.mri.result.now_dst_date = now_dst_date;
		Moira.mri.result.now_end_degree = now_end_degree;
		Moira.mri.result.now_loc = now_loc;
		Moira.mri.result.now_lunar_date = now_lunar_date;
		Moira.mri.result.now_lunar_leap_month = now_lunar_leap_month;
		Moira.mri.result.now_poles = now_poles;	//{壽元干支、子平小運長生干支、年柱、月柱、日柱、時柱}
		Moira.mri.result.now_ref_ut = now_ref_ut;
		Moira.mri.result.now_sign_alt = now_sign_alt;
		Moira.mri.result.now_sign_azimuth = now_sign_azimuth;
		Moira.mri.result.now_sign_display_sort_orders = now_sign_display_sort_orders;
		Moira.mri.result.now_sign_pos = now_sign_pos;	//行星位置。傳入planet_no
		Moira.mri.result.now_sign_state = now_sign_state;
		Moira.mri.result.now_sign_status = now_sign_status;
		Moira.mri.result.now_sign_tip = now_sign_tip;
		Moira.mri.result.now_speed_color = now_speed_color;
		Moira.mri.result.now_start_degree = now_start_degree;
		Moira.mri.result.now_state_color = now_state_color;
		Moira.mri.result.now_table = now_table;
		Moira.mri.result.now_year = now_year;
		Moira.mri.result.now_year_seq = now_year_seq;
		Moira.mri.result.num_asteroids = num_asteroids;
		// Moira.mri.result.NUM_ROW_THRESHOLD=NUM_ROW_THRESHOLD;
		// Moira.mri.result.PALLAS=PALLAS;
		Moira.mri.result.pick_year_info = pick_year_info;
		Moira.mri.result.planets = planets;
		// Moira.mri.result.PLUTO=PLUTO;
		// Moira.mri.result.POLE_OVERRIDE_OFFSET=POLE_OVERRIDE_OFFSET;
		Moira.mri.result.pole_tab = pole_tab;
		Moira.mri.result.power_index = power_index;
		Moira.mri.result.power_key = power_key;
		Moira.mri.result.primary_speed = primary_speed;
		// Moira.mri.result.PURPLE=PURPLE;
		Moira.mri.result.quick_azimuth = quick_azimuth;
		// Moira.mri.result.SATURN=SATURN;
		Moira.mri.result.season_starts = season_starts;
		Moira.mri.result.self_sign_pos = self_sign_pos;
		Moira.mri.result.show_aspects = show_aspects;
		Moira.mri.result.show_aspects_set = show_aspects_set;
		Moira.mri.result.show_compass = show_compass;
		Moira.mri.result.show_fixstar = show_fixstar;
		Moira.mri.result.show_fixstar_set = show_fixstar_set;
		Moira.mri.result.show_gauquelin = show_gauquelin;
		Moira.mri.result.show_gauquelin_set = show_gauquelin_set;
		Moira.mri.result.show_horiz = show_horiz;
		Moira.mri.result.show_horiz_set = show_horiz_set;
		Moira.mri.result.show_now = show_now;
		Moira.mri.result.show_now_set = show_now_set;
		Moira.mri.result.sidereal_stellar_names = sidereal_stellar_names;
		Moira.mri.result.sign_computation_type = sign_computation_type;
		Moira.mri.result.sign_display = sign_display;
		Moira.mri.result.sign_display_orders = sign_display_orders;
		Moira.mri.result.sign_lock = sign_lock;
		Moira.mri.result.sign_opposite = sign_opposite;
		Moira.mri.result.sign_pos_shift = sign_pos_shift;
		Moira.mri.result.sign_prefix = sign_prefix;
		Moira.mri.result.sign_rev_flip = sign_rev_flip;
		Moira.mri.result.signs = signs;
		Moira.mri.result.single_wheel_mode = single_wheel_mode;
		Moira.mri.result.sky_pole_names = sky_pole_names;
		Moira.mri.result.solar_date = solar_date;
		Moira.mri.result.solar_eclipse = solar_eclipse;
		Moira.mri.result.solar_return_pos = solar_return_pos;
		Moira.mri.result.solar_terms = solar_terms;
		Moira.mri.result.solid_house = solid_house;
		Moira.mri.result.star_equ_map = star_equ_map;
		Moira.mri.result.stellar_names = stellar_names;
		Moira.mri.result.stellar_sign_pos = stellar_sign_pos;
		// Moira.mri.result.SUN=SUN;
		Moira.mri.result.tab = tab;
		Moira.mri.result.ten_god_list = ten_god_list;
		Moira.mri.result.ten_god_list_alt = ten_god_list_alt;
		Moira.mri.result.ten_god_list_org = ten_god_list_org;
		Moira.mri.result.ten_god_mode = ten_god_mode;
		Moira.mri.result.ten_god_seq1 = ten_god_seq1;
		Moira.mri.result.ten_god_seq2 = ten_god_seq2;
		Moira.mri.result.tenth_cusp_pos = tenth_cusp_pos;
		Moira.mri.result.three_danger = three_danger;
		Moira.mri.result.time_adjust = time_adjust;
		// Moira.mri.result.TINY_DEGREE=TINY_DEGREE;
		Moira.mri.result.tip = tip;
		// Moira.mri.result.TRUE_NODE=TRUE_NODE;
		Moira.mri.result.true_north = true_north;
		Moira.mri.result.true_north_magnetic_shift = true_north_magnetic_shift;
		Moira.mri.result.twelve_signs = twelve_signs;
		// Moira.mri.result.URANUS=URANUS;
		Moira.mri.result.use_primary_speed = use_primary_speed;
		// Moira.mri.result.VENUS=VENUS;
		// Moira.mri.result.VESTA=VESTA;
		Moira.mri.result.weak_house = weak_house;
		Moira.mri.result.wife_pos = wife_pos;
		Moira.mri.result.wife_signs = wife_signs;
		Moira.mri.result.work_cal = work_cal;
		Moira.mri.result.year_char = year_char;
		Moira.mri.result.year_data = year_data;
		Moira.mri.result.year_names = year_names;
		// Moira.mri.result.YEAR_POLE=YEAR_POLE;
		Moira.mri.result.year_signs = year_signs;
		Moira.mri.result.year_star_map = year_star_map;
		Moira.mri.result.year_star_range = year_star_range;
		Moira.mri.result.year_star_seq = year_star_seq;

		Moira.mri.result.nowYearPosition = nowYearPosition(current_age);
		Moira.mri.result.good_pattern = good_styles.toString();	//政餘喜格，只能回傳預設數量
		Moira.mri.result.bad_pattern = bad_styles.toString();	//政餘忌格，只能回傳預設數量
    }
}